...
export const TaskListItem: React.FC<TaskListItemProps> = ({ 
    ...
    return (
        ...
        >
            <span 
                className={`absolute material-symbols-rounded -z-1 ${isSelected && animation ? '-translate-x-8' : ''} ease-[cubic-bezier(.68,-0.55,.27,1.55)] transition-all duration-200`}
                style={{ fontSize: '2.0rem', color: isSelected ? accent : 'transparent' }}
            >
                chevron_right
            </span>
            ...
            <div className='bg-[#F8F9FA] w-full h-full flex items-center rounded-4xl justify-between pr-6'>
                <div className='flex items-center gap-4 transition-all duration-200'>
                    ...
                    <div className='flex flex-col'>
                        <div className='flex items-center'>
                            ...
                            <span className={`text-lg font-semibold transition-colors duration-200 ${isCompleted ? 'line-through' : ''}`}>
                                {title}
                            </span>
                        </div>
                        <span className={`text-sm text-gray-700 pl-1 transition-all duration-200 ${isCompleted ? 'line-through' : ''}`}>
                ...
                >
                    {!isCompleted ? (
                        animation ? 'radio_button_unchecked' : (
                            <DotLottieReact
