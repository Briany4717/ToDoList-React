...
const TaskListItem = ({ id, title, description, accent, isCompleted, onToggle, isSelected, onSelect }) => {
  ...
  return (
    <div onClick={() => onSelect(id)}
      className={`${animation ? 'shadow-md group' : '-mt-21 -z-10'} items-center w-full h-18 bg--gray-200 border border-gray-200 rounded-lg flex snap-start ${isCompleted ? '' : 'delay-800'} transition-all duration-300 cursor-pointer relative`}>
      ...
      <div className='bg-[#F8F9FA] w-full h-full flex items-center rounded-4xl justify-between pr-6'>
        ...
        <span
          ...
          className={`material-symbols-rounded items-center justify-center cursor-pointer ${animation ? 'hover:text-blue-400 hover:scale-110' : ''} 
            transition-all duration-200 overflow-scroll text-3 ${isCompleted ? (animation ? 'text-green-500' : '') : ''}`}
...
const TiledMenu = ({ children, tiles, selectedTile, onSelect }) => {
  return (
    <div className='flex flex-col w-full'>
      ...
      <div className='flex flex-col h-140 overflow-visible overflow-y-scroll px-6 pt-3 gap-3 border-t-1 border-gray-300'>
        {children}
      </div>
    </div>
  )
